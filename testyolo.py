...
def yolo_video(readvideo):
    ...
    #mydata
    # video_name = (path.split('\\')[1]).split('.')[0]  #video本身名字
    # which_fil = (path.split('\\')[0]).split('/')[2] #放在01還是02
    # print(video_name)
    # new_path = "./video/yolov8/"+which_fil+'/'+video_name+"/"+video_name+'.avi' #新輸出的影片
    # output_folder = "./video/yolov8/"+which_fil+'/'+video_name+'/'
    ...
    while True:
        ...
        results = model(img, stream=True, device = 'gpu')
    ...
    #output_json = './video/yolov8/'+which_fil+'/'+video_name+"/"+video_name+'.json'  #mydata用的
...
if __name__ == '__main__':
    ...
    #yolo_video('./video/531-5.mp4')
    yolo_video('./video/myData0901-1.mp4')

    end_time = time.time()
    print("程式執行時間:", end_time-start_time)

    # target_file = './video/02'
    # file_list = read(target_file)
    # for file in file_list:
    #     now_video = os.path.join(target_file, file)
    #     print("video:", now_video)
    #     yolo_video(now_video)
